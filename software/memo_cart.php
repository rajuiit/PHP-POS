...
if(isset($_POST['page']) && !empty($_POST['page']))
{
	...
	if($_POST['page'] == "submit_cart")
	{
		...
		$vpb_check_items = mysqli_query($db,"select * from `shop_products_memo` where `username` = '$sess_admin_user_id'");
...
$current_stock	=	get_product_current_stock_info($product_id)-$quantity;
		...
		$query6=	mysqli_query($db,"INSERT INTO `shop_stock` (
			...
			$q5		=	mysqli_query($db,"SELECT sum( `amount`) as total FROM `shop_products_memo` where `username` = '$sess_admin_user_id'");
	...
	$db = new mysqli(SERVER, USERNAME, PASSWORD, DATABASE);
			...
			mysqli_query($db,"delete from `shop_products_memo` where `username` = '$sess_admin_user_id'");
			?>
			<div id="vpb_shopping_cart_is_currently_empty" align="left">
...
<div align="center" style="font-size:9px;">Powered by: Zubair Bin Tareque<br>Email: zubaireye@gmail.com</div>
	...
	elseif($_POST['page'] == "remove_this_item") //Remove a specific item from the cart
	{
		...
		mysqli_query($db,"delete from `shop_products_memo` where `id` = '".strip_tags($_POST["item_id"])."' and `username` = '$sess_admin_user_id'");
		$vpb_check_items = mysqli_query($db,"select * from `shop_products_memo` where `username` = '$sess_admin_user_id'");
		if(mysqli_num_rows($vpb_check_items) < 1)
		{
		...
		else
		{
			...
			$vpb_check_itemsTotal = mysqli_query($db,"select sum(amount) as `items_total` from `shop_products_memo` where `username` = '$sess_admin_user_id'");
	...
	if(isset($_POST['product_id']) && !empty($_POST['product_id']) && isset($_POST['product_name']) && !empty($_POST['product_name']) && isset($_POST['product_price']) && !empty($_POST['product_price']))
		{
			...
			//Check if a specified user has already added a specified item to cart by checking the database shop_products_memo's table
			
	$db = new mysqli(SERVER, USERNAME, PASSWORD, DATABASE);

$product_name12=get_product_info("product_name",$_POST['product_name']);

	$vpb_check_items = mysqli_query($db,"select * from `shop_products_memo` where `username` = '$sess_admin_user_id' and `item_added` = '$product_name12'");

			//If the specified user has not already added the specified item to database shop_products_memo's table

			

			if(mysqli_num_rows($vpb_check_items) < 1)
			{
				//Add the specified item to the database shop_products_memo's table now
...
$add_date			=	date('Y-m-d');
			...
			mysqli_query($db,"insert into `shop_products_memo`(
															`id` ,
															`product_id` ,
															`username` ,
															`item_added` ,
				...
				//Check all the items that the specified user has added to the database shop_products_memo's table
				
					$db = new mysqli(SERVER, USERNAME, PASSWORD, DATABASE);

				$vpb_check_all_items = mysqli_query($db,"select * from `shop_products_memo` where `username` = '$sess_admin_user_id' order by `id` asc");
				...
				{
					//Check the databse shop_products_memo's table and sum up the total of all added items to cart
	...
	$db = new mysqli(SERVER, USERNAME, PASSWORD, DATABASE);
					...
					//Update shop_products_memo's table to avoid repetition of a specified item and increment items quantity and amount to display new info

$product_name12=get_product_info("product_name",$_POST['product_name']);

					mysqli_query($db,"update `shop_products_memo` set
