...
$product_id=filter_input(INPUT_GET, 'id', FILTER_SANITIZE_SPECIAL_CHARS);
		...
		$q	=	mysqli_query($db,"SELECT * FROM `shop_stock` WHERE status=1 AND sales!=0 AND (add_date BETWEEN '$date1' AND '$date2') GROUP BY product_id ORDER BY add_date ASC");
	...
	<tr>
		...
		<td><div align="center"><?php echo get_sales_summary_product_id($r['product_id']); ?></div></td>
