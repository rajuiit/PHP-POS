...
$stock_search	=	filter_input(INPUT_POST, 'stock_search', FILTER_SANITIZE_SPECIAL_CHARS);

$db = new mysqli(SERVER, USERNAME, PASSWORD, DATABASE);
$query	=	mysqli_query($db,"SELECT * FROM `shop_product` WHERE product_code='$stock_search'");
...
?>
	...
	<input type="hidden" name="stock_search" value="<?php echo $stock_search; ?>" class="form-control" />
						</div>
					</div><!-- form-group -->
					...
					<div class="form-group">
						...
						<div class="col-sm-8">
						<input type="text" name="remarks" value="Return Stock <?php echo get_product_info("product_name",$product_id); ?>" class="form-control" />
						</div>
					</div><!-- form-group -->
