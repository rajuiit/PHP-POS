...
<script type="text/javascript">
        function PrintDiv() {
            ...
            frameDoc.document.write('<h1 style=text-align:center;>Item Wise Sales Report</h1>');
...
$product_id=filter_input(INPUT_GET, 'id', FILTER_SANITIZE_SPECIAL_CHARS);
		...
		$q	=	mysqli_query($db,"SELECT * FROM `shop_stock` WHERE status=1 AND product_id=$product_id AND sales!=0 AND (add_date BETWEEN '$date1' AND '$date2') ORDER BY add_date ASC");
	...
	<tr>
		...
		$price	=	get_product_info("product_price",$r['product_id']) * ($r['sales']);
