...
<script type="text/javascript" language="javascript" src="memo.js"></script>
<link type="text/css" rel="stylesheet" media="all" href="vasplus_programming_blog_shopping_cart_v4.css" />


<div id="vasplus_programming_blog_cart_titles" class="shopping_cart_status">Memo Item List</div>

<?php

$db = 	new mysqli(SERVER, USERNAME, PASSWORD, DATABASE);
$qq	=	mysqli_query($db,"SELECT `memo_id` FROM `shop_memo` ORDER BY `memo_id` desc limit 0,1");

$rr	=	mysqli_fetch_array($qq);

$memo_id	=	$rr['memo_id']+1;
?>
	
<div style="width:50px;float:left;" align="right">
	<form action="memo_invoice.php?" target="blank" method="POST">
<input type="hidden" name="memo_id" value="<?php echo $memo_id; ?>">
	
	<input type="submit" name="action" value="Save_Print_Preview" title="Save" id="vasplus_p_blog_add_to_cart_button" />
	</form>
</div>	
...
if(isset($_POST['p_price'])){
	...
	//echo $change_price;
	$change_q = mysqli_query($db,"UPDATE `shop_products_memo` SET 
															`price` = '$change_price' ,
...
else
{
	...
	$check_itemsTotal = mysqli_query($db,"select sum(amount) as `items_total` from `shop_products_memo` where `username` = '$sess_admin_user_id'");
